...
* 1.a) What is the purpose of this program? Creating sierpinski triangles where it 'hops' randomly half way in between each corner.
* 1.b) Does the program produce output? If so, what kind of output is it, and where does it end up? It is an html file, in the project file.
* 1.c) Does the program accept input? If so, what kind of input, and how is it provided? the int used in doTriangle(int).
...
* 2.a) List all the ways that you can see that the current output of your program is different from the expected output.
 Does not appear to be bounded by a triangle. Created with crosses instead of 8's.
Step 3
------
...
* 3.a) Explain one or two methods you could use to determine which of the three parts of the program is causing this first bug. (In step 4, I will tell you how I would do it.)
    By placing break points just before and after the making the grid section, we can see how grid is behaving and determine what it actually holds.
...
* 4.a) What would you expect grid to contain before the bug starts hopping? What does it actually contain at this point? expected to be 8's, instead it is filled with 'f's.
...
* 6.c) What is the bug? semi colon before loop.
...
* 7.a) What problems were fixed in the preceding steps? What errors can we see now? We have the spacing correct, but we do not have the 8's.
...
* 8.a) What should grid look like at the end of the first stage? What does it actually look like? a few 8's and spaces.
* 8.b) What should grid look like at the end of the second stage? What does it actually look like? all the way across with 8's and spaces. 
* 8.c) Which section is the current bug in? printing.
...
* 9.a) What is the bug in the printing loop? its adding i instead of sideLength+j.
...
* 10.a) What is wrong with the picture now? the top point of triangle is not centered.
...
* 11.a) Where is the bug, and how should you fix it? instead of sideLength-1, it should be sideLength/2.0 to center it.
...
we would have had to use break points and look at every variable.
...
* 12.a) What is still wrong with the output? The triangle is slanted over.
...
Middle Section where triangle is being created.
...
* 14.a) What was the bug? needs to have a break in case 0, as it does in the others.
