...
To generate a sierpinski's triangle given the resolution, and to output the generated image to an html file.
* 1.b) Does the program produce output? If so, what kind of output is it, and where does it end up?
Yes, it outputs text with html tags to an html file called "triangle.html".  I believe it will end up in the project directory that I have created.
* 1.c) Does the program accept input? If so, what kind of input, and how is it provided?
It accepts one input parameter that is an integer.  It is provided to the doTriangle function and will determine the resolution of the Sierpinski's Triangle.
...
One way to do it would be to set breakpoints before it reaches a certain part of the source code while using the debugger to see which part has incorrect or unexpected values. Or you could create a control flow diagram to try and test which path might be causing the error.
...
An empty space or character(I think the value 32). It contains the character I with an accent on it or numerically -51.
...
I believe the setup part, it already has issues with the character values before we reach the next steps.
Step 5
------
...
Now we have fixed the strange characters from appearing and are correctly having spaces appear.  Now the errors are that the '8's are showing up incorrectly, they only show up as solid lines towards the bottom of the file.
Step 8
------
...
It should contain almost all spaces, with a couple of "setup" locations of the bug.  It does actually look like this in the text visualizer.
* 8.b) What should grid look like at the end of the second stage? What does it actually look like?
It should look complete, or have a mix of spaces and '8's.  It does actually look like this and is pretty evenly diverse.
* 8.c) Which section is the current bug in?
I think the bug is currently in the printing stage or the third stage, because the first two stages seem to have the correct output or values in grid.
...
I think the bug may be that instead of adding i to the sidelength we should be adding j.  Or else the value just keeps adding to the same position for 200 iterations.
...
The bug is on line 79, I think that it is putting the corner in the top right instead of the center. And to fix it we would have to divide the sideLength in half.
...
I would have looked at the previous working corners, and probably altered the variables to see which changes affect what output.
...
Although more triangle shaped overall, most of the triangles are still skewed to the right.
...
I think the "part 2" portion of the code.  I think the the picking the random corner area is the issue, potentially the random point or the switch statements.
