...
In this program we create a Sierpinski triangle that is represented with 8's.

* 1.b) Does the program produce output? If so, what kind of output is it, and where does it end up?

Yes it produces a visual representation of a Sierpinski triangle in a .html file. 

* 1.c) Does the program accept input? If so, what kind of input, and how is it provided?

No it currently does not accept input. It could be made to accept input.
...
Mine is a giant grid of what looks like a crosses. Interspersed in there are lines of 8's as well. There is no white space on the grid.
...
You could make a for loop to test each character in the grid and see if it was either a space or an eight. If it wasn't something went wrong.
...
I would expect the grid to have nothing in it, but it is filled with accented i's and an 8 at the end. 
...
Setting up the first triangle must have the first bug, as there are no white spaces.
...
No skips to setting up p1x through p3y.

* 6.c) What is the bug?

There is a semi-colon at the end of the for statement, before the loop.
...
* 7.a) What problems were fixed in the preceding steps? What errors can we see now?
 We got rid of all of the junk characters. However we now have random lines of 8's.
Step 8
------
...
The grid should be completely blank after the first stage of the program. This is correct.

* 8.b) What should grid look like at the end of the second stage? What does it actually look like?

The grid should have the Sierpinski triangle shown by 8's. This is possibly correct, it shows a mix of blanks and 8's.

* 8.c) Which section is the current bug in?

It is in the printing of the grid.
...
When we print the grid, we are printing whatever the first character is for the row, instead of what character should be at the row x column.
...
Everything is smashed up against the right side of the page. We have our trinagles correct, but the spacing is incorrect.
...
We will need to adjust the x value to be in the center, not on either side.
...
If there were no comments, I could have looked at the vairable names to find the bug. That would have taken a long time. Had there been no comments, and bad variable names, it would have been near impossible to find the bug.
...
It seems that we only have the right sides of all the Sierpinski triangles.
...
I couldn't find it, so I looked at the next step.
...
In the switch statement there was no "break" in case 0. Therefore it went through case 0 and case 1 at the same time.
