...
* 1.a) What is the purpose of this program?
	The purpose of this program is to create an HTML document that displays sierpinski's triangle.
* 1.b) Does the program produce output? If so, what kind of output is it, and where does it end up?
	It produces output in the form of an html document and it places it in the project folder.
...
* 2.a) List all the ways that you can see that the current output of your program is different from the expected output.
	The current output is one huge block of T's and 8;s.
Step 3
------
...
* 3.a) Explain one or two methods you could use to determine which of the three parts of the program is causing this first bug. (In step 4, I will tell you how I would do it.)
	The problem seems to lie in filling out the grid in that instead of spaces it includes T's.
...
* 4.a) What would you expect grid to contain before the bug starts hopping? What does it actually contain at this point?
	It should contain only spaces and 8s however it includes strange characters instead of the spaces.
...
The setup of the grid is where the strange characters start and must be where the problem lies.
Step 5
------
...
* 6.c) What is the bug?
	The loop that assigns the space values only runs once instead of the 39999 times it is supposed to run.
...
* 7.a) What problems were fixed in the preceding steps? What errors can we see now?
	Now there are only spaces and 8s, however they don't create the triangle we want.
Step 8
------
...
* 9.a) What is the bug in the printing loop?
	the outfile is only changed by a changing i and not the j and i. changed 119 from 	outfile << grid[i*sideLength + i]; to 	outfile << grid[i*sideLength + j];
Step 10
-------
...
* 10.a) What is wrong with the picture now?
	the picture is not centered at all.
Step 11
-------
...
* 11.a) Where is the bug, and how should you fix it?
	the bug is on line 79. It should be sideLength/2 instead of sideLength-1.
* 11.b) If I had not commented my code, how would you have discovered which variable to fix? What if I had used crazy variable names like “fooa, foob, fooc, food, fooe, foof” instead of “p1x, p1y, p2x, p2y, p3x, p3y”?
	It would be much harder to debug, the best option there would be to set a breakpoint at the beginning of populating  grid and step through until a problem is found.
...
* 12.a) What is still wrong with the output?
	Only the right half of the triangle is outputted.
Step 13
-------
...
* 13.a) Which portion of the code do you think is causing the problem? If you aren’t sure, you can try repeating step 4, as well as looking over the code.
	The error seems to be in the generation of the triangles.
...
* 14.a) What was the bug?
	When case 0 runs it also runs case 1. There is no break;
