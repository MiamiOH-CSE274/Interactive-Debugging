...
Produces an html file in the project folder.
* 1.c) Does the program accept input? If so, what kind of input, and how is it provided?
It accepts the size of the triangle in an int.
...
Well it doesn't make a triangle first of all. Secondly there are a bunch of random "i" characters.
...
You could break during those parts to see if it's doing what's actually expected.
...
We'd expect it to contain spaces and 8's but it contains strange characters.
...
Making the triangle has the bug.
...
Lines 63-66 set the grid full of spaces.
...
No
* 6.c) What is the bug?
There is a semicolon after the loop
...
The triangles are not triangles.
...
grid[i*sideLength +i] should be grid[i*sideLength +j]
...
The triangle isn't a triangle still, and it's all on the right.
...
The bug is in the setting vertexes, find the correct variable and fix it.
...
I would have looked through the code to discover where the variables were being used.
...
Case 0 fell through to case 1 because it was missing a break.
