...
The program creates a visual representation of the Sierpinski triangle using ASCII letters.
* 1.b) Does the program produce output? If so, what kind of output is it, and where does it end up?
The program creates a HTML file that contains a Sierpinski triangle using ASCII letters.
* 1.c) Does the program accept input? If so, what kind of input, and how is it provided?
The program accepts a input for the size of the Triangle, recieveing the imput as a parameter in the dotTriangle() method.
Step 2
-------
...
The output does not have a Sierpinski triangle inside it. It contains a large square of text, that does not resemble any triangle. There are also six lines of '8's and the rest of the lines are simply '1's.
...
I would begin to search for the problem by using a series of std::cout statements to make sure each part of the code is executed. If it is all executed, I would then check the values of "grid" while the data is being put into "grid", as well as when it is being printed. If the numbers are completely different, there is a problem with reading memory not in the array. The second method I would use to solve the problem would be to use the actual debugger to check to make sure everything is initilized, added to, and printed out in the correct way.
...
I expected the grid to contain a large number of empy spaces, and it instead contained a large number of characters that look like 'I' but are different, I do not know the name of the char.
...
It looks like the setup part of the program is the issue, because grid is supposed to contain spaces, but contains a different character.
Step 5
------
...
The while loop in line 63 to line 66 appear to set the grid to be full of spaces.
Step 6
------
...
I expected to have to use "step over" once, as "step into" normally goes line by line in debuggers before, so there would have to be something different between the two commands.
* 6.b) Try hitting “step over” repeatedly, and see if it does what you expected.
It only takes one click when you set the breakpoint at the line.
* 6.c) What is the bug?
The for loop has a semi colon at the end of it.
...
* 7.a) What problems were fixed in the preceding steps? What errors can we see now?
 Now, all of the strange characters were replaced by spaces. However, there are still lines of '8's that do not form any kind of triangle.
Step 8
------
...
It is only printing the first value in the loop for each line, as 1 is a constant.
...
Only half of the triangle is being printed.
...
The bug is the line giving p3x a value. I would fix it by dividing sideLength by 2, to put the corner in the middle of the triangle.
...
I would have looked for any variable with a name that is similar to what I am looking for. If the variable did not have a normal name, it is doubtful that I would be able to find it without resorting to a long period of trial and error.
