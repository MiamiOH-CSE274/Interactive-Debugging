...
* 1.a) What is the purpose of this program?
     The purpose of this program is to build a sierpinski triangle into a new file with what ever size you want
* 1.b) Does the program produce output? If so, what kind of output is it, and where does it end up? The program produces does not have an output, but it does have a side effect and that is that a sierpinski triangle is printed into and html file
* 1.c) Does the program accept input? If so, what kind of input, and how is it provided? The program accepts an integer as an input. It is provided by just typing a number into the main before you run it
...
* 2.a) List all the ways that you can see that the current output of your program is different from the expected output. The output file 
* is just a square of characters, not everything is an 8. There aren't any triangles
...
* 3.a) Explain one or two methods you could use to determine which of the three parts of the program is causing this first bug. (In step 4, I will tell you how I would do it.) I will set breakpoints at the end of the first part of the program
* to see if the problem is in the setting up the problem section, next I would put a breakpoint after the second section to see if 
* the problem is in the printing section
...
* 5.a) Which lines in the code look like they should set the grid to be full of spaces? It looks like lines 63-65 are where the array should be set to blank spaces.
...
* 11.a) Where is the bug, and how should you fix it? The bud is that the code is setting the top middle vertex to be out along the top right of the page
* 11.b) If I had not commented my code, how would you have discovered which variable to fix? What if I had used crazy variable names like “fooa, foob, fooc, food, fooe, foof” instead of “p1x, p1y, p2x, p2y, p3x, p3y”? I would have been screwed
...
* 13.a) Which portion of the code do you think is causing the problem? If you aren’t sure, you can try repeating step 4, as well as looking over the code. I think the problem with the program is in the first 85 lines, that is where not many 8's are being printed out
