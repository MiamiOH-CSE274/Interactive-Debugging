...
-The current output doesn’t look like a triangle at all.  It’s a page full of what looks like little arrows, and then some 8’s at the bottom of the page.
...
-A method such as isValidChar() could check the chars that are put into the grid, and check to see that they are all either spaces or 8’s.
...
-Shouldn’t contain anything before the bug hops.  Right now it contains a lot of capital I’s with tildes over them, and a few 8’s.
...
-I think it must be in set up.  The strange characters popped up after that step, then it looks like making the triangle inserted some 8’s into the grid, meaning it is at least partially working.
...
-40,000 to fill in every space of the grid.
* 6.b) Try hitting “step over” repeatedly, and see if it does what you expected.
-The debugger jumped out of the for loop and onto more code.  Not what I expected.
* 6.c) What is the bug?
-The semi colon in the loop is throwing it out from iterating again.
...
-All of the strange characters are gone now.  The 8’s still aren’t forming a triangle though.
...
-A lot of spaces with a few 8’s spread out. Looks like expected
* 8.b) What should grid look like at the end of the second stage? What does it actually look like?
-A few more 8’s still very spread out. Expected this.
* 8.c) Which section is the current bug in?
-The bug is probably left in the printing step.
Step 9
------
...
-I isn’t changing at all, not affecting the loop.
...
-There is somewhat of a structure being printed, but it’s only on the right side of the html page.
...
Side length -1 does not get you to the center of the grid.  Probably divide by 2 instead.
...
-Probably by noticing that we are assigning 8 to 3 points of the grid in this section, then looking at the logic used when Assigning the points.  The crazy variables would have been confusing, guess that’s why we have to choose logical variable names!
...
-Only half of the triangle is printed, and it still looks strange.
...
-I’m not sure where the bug is.  It might be a problem like creating the grid on only half of the page.
...
-There is no break; statement at the end of case 0.
