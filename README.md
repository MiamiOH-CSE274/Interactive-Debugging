...
The purpose of this program is to generate a Sierpinski triangle.
* 1.b) Does the program produce output? If so, what kind of output is it, and where does it end up?
The program produces a Sierpinski triangle that ends up as html that is printed to the file named triangle.html
* 1.c) Does the program accept input? If so, what kind of input, and how is it provided?
The input accepted is an integer and is provided by the programmer by passing an integer value to doTriangle.
Step 2
-------
...
- You can open the current and the expected html file and compare them.
- We can read through the code and try to figure out the logic for each step to see if it is correct.
- Use the debugger.
Step 3
------
...
- We could debug each section from the 1st to the 3rd.
- Look through the code to see if we can identify any general errors that are not very specific to the program but rules of C++. 
Step 4
------
...
Each entry should be a space but it contains a bunch of Í with some 8 in all of them.
...
The setup contains the first bug. We know this because the program is not even printing out just spaces which is in the setup section.
Step 5
------
...
Lines 60 - 85, line 65 sets grid[i] = ' ';
Step 6
------
...
* 7.a) What problems were fixed in the preceding steps? What errors can we see now?
 We fixed the problem of generating Í and now we have some spaces and some 8's but they do not form a triangle as desired.
Step 8
------
...
At the end of section one we should expect mostly spaces with a few 8's. Which are at the first, second and third corner. However, there are a few rows of 8's and many rows of just spaces.
* 8.b) What should grid look like at the end of the second stage? What does it actually look like?
At the end of section two we should expect spaces and 8's (at random spots). Instead we have rows of 8's only and rows of spaces only.
...
* 9.a) What is the bug in the printing loop?
 In the grid[i*sideLength + i]; section we need to change it to grid[i*sideLength + j]; because it was never changing the position in the grid[].
Step 10
-------
...
The triangle is starting in the top right corner.  
Step 11
-------
...
The bug is on line 79, double p3x = sideLength-1; is what is causing the error.
...
We could have used the debugger to step through the code to check and see what points made up fooa, foob... and seen what points we have and what points we don't have. Once we know the values we change them accordingly to get the desired points/values we want for our 3 locations.
...
We have the third point which appears to be in the correct spot, however, all 8's are mainly to the right of the 3rd point. 
Step 13
-------
...
Most likely the bug is in the section where the triangle is being build. 
...
break; was not after the first case (case 0). 
